...
→ Prompt → 关键帧 → Shot 视频 → Timeline / BGM / 字幕 → Remotion 成片
```
...
| 需求 | 实现与验收 |
|---|---|
| 本地、画布视频、分享 URL | 支持 `.mp4`、`.mov`、`.webm`，画布视频“用作参考视频”，分享文案 URL 提取、Media Resolver、本地化和重试 |
...
| 关键帧 | simple 生成 Start，medium 生成 Start + End，complex 生成 Start + Middle + End；支持批量并发、单帧重试和确认门 |
| Shot 视频 | 按模型能力选择首尾帧或多参考图协议，把动作、运镜、时序、对白、环境音与动作音效写入 Prompt；支持单 Shot 重试和确认门 |
...
| Remotion 输出 | 复用现有 Manifest / Renderer，支持渲染进度、取消、失败恢复和重复渲染更新唯一 Final Video Node |
| 持久化与恢复 | Video Remix 全状态写入项目；旧 schema 自动补默认值；平台提交未知时不盲目重投，安全的本地任务可恢复 |

## 验收案例

自动回归使用一条等价于 18 秒参考片的结构化 fixture：3 个 Shot、2 个人物、2 个场景、
1 个道具和 1 句对白。测试依次完成 CHAR、SCENE、PROP 替换，simple / medium / complex
关键帧计划与确认，全部 Shot 视频确认，Timeline、上传 BGM、字幕、Manifest、渲染状态，
最后断言画布只有一个 Final Video Node。

此外使用本地 2 Shot 媒体做了真实 Remotion 冒烟验收：输出为 640×360、H.264 + AAC、
时长 2.261 秒，两个 Shot 和两段字幕均可见；刷新项目后仍保持完成状态和唯一成片节点。
...
“展开镜头到画布”在需求中标记为高级功能，不属于第一版 MVP 清单，本版保持单个
Video Remix 子工作流容器，后续再按需生成独立 Shot Node。

## 最终验证结果

- Video Remix 专项：68 项全部通过。
- 全仓回归：579 项全部通过。
- TypeScript：`tsc --noEmit` 通过。
- 生产构建：Vite 成功转换 2373 个模块并产出前端资源；仅保留项目既有的大 Chunk 提示。
