 ...
 *
 * Google Flow 文生图/图生视频、即梦视频生成都操作同一个系统共享 Chrome，
...
export function enqueueGoogleFlowWorkflow(task, {
    ...
    // assertBrowserWorkflowIdle 随即放行「打开共享 Chrome / 检查登录」——
...
// 语义化别名：队列锁定同一个系统共享 Chrome，不是 Google Flow 专属。
 ...
 * 所以生成期间点一次「打开系统共享 Chrome」会一直转到整个生成结束。
 */
export function assertBrowserWorkflowIdle(action) {
    const label = browserWorkflowBusyLabel();
    if (!label) return;
    const error = new Error(`系统共享 Chrome 正在执行「${label}」，现在${action}会中断当前生成。请等它完成后再试。`);
