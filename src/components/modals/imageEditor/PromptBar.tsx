    ...
    return (
        <div className="w-full bg-[#2a2a2a] bg-opacity-95 backdrop-blur-sm rounded-xl border border-neutral-600 shadow-2xl pointer-events-auto flex items-center px-3 py-2.5 gap-3">
            ...
            <div className="relative flex-shrink-0" ref={modelDropdownRef}>
                ...
                >
                    {currentModel.provider === 'workflow' ? (
                ...
                {showModelDropdown && (
                    <div className="absolute bottom-full mb-2 left-0 w-48 bg-[#252525] border border-neutral-700 rounded-lg shadow-xl overflow-hidden z-50">
                        ...
                        {availableModels.filter(m => m.provider === 'codex').length > 0 && (
                            <>
                                <div className="px-3 py-1.5 text-[10px] font-bold text-neutral-500 uppercase tracking-wider bg-[#1f1f1f]">本地生图</div>
                                {availableModels.filter(m => m.provider === 'codex').map(model => (
                                    <button
                                        key={model.id}
                        ...
                        {availableModels.filter(m => m.provider === 'workflow').length > 0 && (
                            <>
                                <div className="px-3 py-1.5 text-[10px] font-bold text-neutral-500 uppercase tracking-wider bg-[#1f1f1f] border-t border-neutral-700">Evan 专属 Chrome（Flow / 即梦 / Gemini）</div>
