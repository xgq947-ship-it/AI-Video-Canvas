...
export const CinematicDirectorNode: React.FC<BaseProps & { onRun: (id: string) => void }> = props => {
  ...
  return (
    <Shell {...props} icon={<Film size={20} />} label={data.title || '电影短片导演'} subtitle={data.cinematicDirector?.status === 'running' ? '导演 Skill 执行中…' : `AI 自动分镜 · ${state.totalDuration} 秒 · ${selectedModel?.name || state.videoModel}`} minHeight={CINEMATIC_DIRECTOR_NODE_HEIGHT}>
      <div className="space-y-3 px-5 py-4">
        ...
        <div className="grid grid-cols-2 gap-2"><Field dark={dark} label="总时长（秒）"><Input dark={dark} type="number" min={4} max={600} value={state.totalDuration} onChange={event => update({ totalDuration: Number(event.target.value) })} onPointerDown={stop} /></Field><Field dark={dark} label="视频生成模型"><Select dark={dark} value={state.videoModel} onChange={event => update({ videoModel: event.target.value })} onPointerDown={stop}>{videoModels.map(model => <option key={model.id} value={model.id}>{model.name}</option>)}</Select></Field></div>
        <Field dark={dark} label="台词"><Select dark={dark} value={state.dialogueMode || 'auto'} onChange={event => update({ dialogueMode: event.target.value })} onPointerDown={stop}><option value="auto">AI 自动创作台词</option><option value="preserve">仅保留剧本原对白</option><option value="none">无台词</option></Select></Field>
        <div className={`flex items-center gap-2 rounded-xl px-3 py-2 text-[10px] ${dark ? 'bg-white/[0.04] text-neutral-400' : 'bg-neutral-50 text-neutral-500'}`}><Sparkles size={12} className="text-cyan-400" />分镜数量与每镜头时长由 AI 根据剧情自动决定（限当前视频模型支持的时长档位）</div>
