 ...
 *
 * 规则：
 * - 每个子文件夹名即为素材分类（Character / Scene / Item / Style / Sound Effect / Others…）。
...
export function scanAssetLibrary(assetsDir, { logger = console } = {}) {
    ...
    for (const dirent of fs.readdirSync(assetsDir, { withFileTypes: true })) {
        ...
        for (const file of files) {
            ...
            const url = `/library/assets/${category}/${file.name}`;
            seenUrls.add(url);
            ...
            if (prev) {
                ...
                result.push({ ...prev, category, type, url });
            } else {
                let createdAt = new Date().toISOString();
                try {
                    createdAt = fs.statSync(path.join(catDir, file.name)).mtime.toISOString();
                } catch {
                    /* 用当前时间兜底 */
                }
                result.push({
