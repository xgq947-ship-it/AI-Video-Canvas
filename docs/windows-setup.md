...
```
launcher-windows\Evan工作台.bat
```

### 发送到桌面

在文件资源管理器里找到它 → **右键 → 显示更多选项 → 发送到 → 桌面快捷方式**。

以后双击桌面上那个快捷方式就行。

> ⚠️ **不要直接把 `.bat` 文件本身复制到桌面** —— 它靠自己的位置来定位项目根目录，
...
**日常用法：双击 → 输入 `1` → 回车。** 服务没起会自动拉起，然后自动打开浏览器。

### 几个要点

- **服务是脱离窗口运行的**：启动后可以直接关掉这个菜单窗口，
  前后端仍在后台跑。想停就再打开菜单选 `3`。
- **日志在 `logs\dev-server.log`**：服务在后台跑，报错不会显示在屏幕上，
  出问题就用菜单的 `6` 看日志。
- **首次运行如果没装依赖**，`.bat` 会自动执行 `npm install`（要等 5–15 分钟）。
...
| 现象 | 原因 / 处理 |
|---|---|
| 双击后窗口一闪而过 | 说明报错了。改用：在项目目录开 PowerShell 跑 `npm run launcher`，就能看到报错 |
| 提示「没有找到 package.json」 | `.bat` 被复制到别处了。删掉，回项目里用「发送到 → 桌面快捷方式」 |
| 提示「找不到 Node.js」 | 回步骤 2；装完要**新开窗口** |
| 中文显示成乱码 | `.bat` 里已经有 `chcp 65001`。若仍乱码，改用 Windows Terminal 打开 |
| 选 2 启动后一直转圈到 60 秒失败 | 用菜单 `6` 看日志，多半是端口被占或 `.env` 有语法错误 |
