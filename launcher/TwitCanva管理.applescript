...
on frontendRunning()
	try
		...
		return pageText contains "<title>Evan"
	on error
		return false
	end try
end frontendRunning
...
on startService()
	...
	on error errorMessage
		display dialog ("启动失败：" & errorMessage & return & return & "请查看日志：" & logDir) with title "Evan工作台" buttons {"好"} default button "好"
		return false
	end try
	repeat 30 times
		delay 0.5
...
on openCanvas()
	if not (frontendRunning() and backendRunning()) then
		...
		if not ok then
			display dialog ("服务未能在15秒内完全启动。" & return & "请打开日志查看原因：" & logDir) with title "Evan工作台" buttons {"打开日志", "好"} default button "好"
...
on showStatus()
	...
	display dialog ("前端：" & frontStatus & return & "后端：" & backStatus & return & "服务托管：" & agentStatus & return & return & "项目：" & projectPath) with title "Evan工作台 · 状态" buttons {"打开画布", "好"} default button "好"
...
on run
	repeat
		...
		set selectedItem to choose from list optionsList with title ("Evan工作台  " & statusLine) with prompt ("当前状态：" & statusLine & return & return & "请选择操作：") default items {"打开画布"} without empty selection allowed
		...
		if actionName is "打开画布" then
			...
			if startService() then
				display notification "服务已启动" with title "Evan工作台"
			else
				display notification "启动未完成，请查看日志" with title "Evan工作台"
			end if
		else if actionName is "停止服务" then
			if stopService() then
				display notification "服务已停止" with title "Evan工作台"
			else
				display notification "部分进程仍在运行，请查看状态" with title "Evan工作台"
			end if
		else if actionName is "重启服务" then
			my stopService()
			if startService() then
				display notification "服务已重启" with title "Evan工作台"
			else
				display notification "重启未完成，请查看日志" with title "Evan工作台"
			end if
