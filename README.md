...
商品详情一键复刻在现有生成管线前提供“详情拼接重新切片”预处理：零散竞品图先无损
拼接，再按视觉过渡与当前模型宽高比自动规划切割线，并保证单张切片不超过 1500px。
确认后只替换竞品输入，原始切片
保留在画布中并可一键恢复。对于我方产品与竞品产品模具完全一致的场景，详情节点另提供
“同模换色直出”：直接锁定竞品产品的轮廓、比例、位置、透视与遮挡，只替换我方颜色、
材质纹理、模特、文案和 Logo；每页只进行一次付费生图，质检失败后由用户决定是否手动重生成。
