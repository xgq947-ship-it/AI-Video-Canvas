...
> 迁移说明（2026-08-03）：当前产品入口已统一到画布内 `Video Analysis` 节点。本文保留首版
> 独立工作区的阶段验收证据；新的画布适配层复用同一套拆镜、Gemini 分析、生成队列和 Remotion
> 能力，并把结果写成 `global + shots[]` 后自动创建普通画布节点。
...
→ Prompt → Shot 视频 → Timeline / BGM → Remotion 成片

可选增强：人物/场景/道具三图一致性包，以及起始 / 中间 / 结束关键帧。
```

首版验收时，入口是应用顶部的独立“短视频复刻”工作区，一个项目可保存多个复刻任务；
这部分内容作为历史验收基线保留。当前入口已按上面的迁移说明统一到画布内节点，旧项目数据
仍通过兼容记录和幂等迁移打开。

本地画布、导入、预处理、编辑、持久化、恢复和 Remotion 渲染不依赖 Gemini 登录。
Gemini 未登录时，只有真实视频理解请求不可用；已有项目和所有本地阶段仍可正常打开、
编辑与导出已具备素材的成片。
...
simple / medium / complex 关键帧计划与确认；随后完成全部 Shot 视频确认、Timeline、上传 BGM、Manifest、渲染状态，
最后断言成片保留在独立任务中，只有执行“发送到 AI 画布”后才出现一个普通 Final Video Node。

此外使用本地 2 Shot 媒体做了真实 Remotion 冒烟验收：输出为 640×360、H.264 + AAC、
时长 2.261 秒，两个 Shot 均可见；刷新项目后仍保持完成状态。独立工作区迁移
...
“展开镜头到画布”当时不属于第一版 MVP 清单；统一画布整改后，分析结果会自动展开为普通
关键帧、镜头视频和 Render 节点，旧版项目级任务仍保留为兼容记录。

## 最终验证结果

- 项目级任务、旧数据迁移、最终输出桥接和素材保护均有专项回归测试。
- 首版历史全仓回归：617 项全部通过；统一画布整改后当前全仓回归：626 项全部通过。
